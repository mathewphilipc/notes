...
\end{equation}
where \begin{equation}
X_\xi = \int_M d^d x \sqrt{|g|} (\xi \cdot d \phi)(x) \frac{\delta}{\delta \phi(x)}
\end{equation}
so the first term is \begin{equation}
\begin{aligned}
X_\xi \cdot \Theta (x) & = \LP \int_M d^d x \sqrt{|g|} (\xi \cdot d \phi)(y) \frac{\delta}{\delta \phi(y)} \RP \cdot \LP - (\nabla \phi(x)) \cdot \epsilon \delta \phi(x) \RP \\
