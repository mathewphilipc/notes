...
\begin{abstract}
Notes on active and passive transformations in physics and geometry.
\end{abstract}
...
\frac{d^2 \vec{x}}{dt^2} = - \frac{GM}{|\vec{x}|^2} \hat{x}
\end{equation*}
...
We begin with the simplest example of a 2D space: Euclidean space $\R^2$ described using Cartesian coordinates. Intuitively we know that this space has a large number of \ti{isometries}, that is, ways that we can move points around while respecting distances and angles and so on. For example, if we take every point in $\R^2$ in Cartesian coordinates and shift each point's $x$ coordinate by a constant \begin{equation*}
(x,y) \mapsto (x + a, y)
\end{equation*}
...
\\
\
You may have seen the set of matrices defined by this constraint before. In general dimensions this is called the \ti{orthogonal group} \begin{equation*}
...
\end{equation*}
Orthogonality in a real vector space is directly analogous to unitarity in a complex vector space.\\
\\
\
...
\ & = \LP x' + \PA{x'}{x} dx + \PA{x'}{y} dy, y' + \PA{y'}{x} dx + \PA{y'}{y} dy \RP.
\end{align*} 
...
\ & = \LP \cos^2 \theta + \sin^2 \theta \RP dr^2  - 2 r \cos \theta \sin \theta d r d \theta + 2 r \sin \theta \cos \theta dr d \theta + (r^2 \sin^2 \theta + r^2 \cos^2 \theta) d\theta^2 \\
\ & = dr^2 + r^2 d \theta^2.
\end{align*}
We finish by emphasizing yet again: here there is no notion of $ds^2$ vs ${ds'}^2$ that we might wish to compare, no notion of whether this transformation respects the geometry. There is automatically no change at all to the geometry, because all we have done is moved from one coordinate description of a space to a different coordinate description of the same space. We could write \begin{equation*}
ds^2 = ds^2
\end{equation*}
but this doesn't tell us something about our transformation. It is vacuously true!
...
\end{equation*}
Two of the defining features of a real inner product are that, thought of as a function of two vectors, it is linear in both of its arguments and gives the same output if we swap the order of its arguments. This means that for any two vectors $\vec{u}, \vec{v}$ we can expand \begin{equation*}
