...
\abstract{Some supplementary discussion of Lagrangians and Noether charges.}
...
\end{equation*}
for $f = f(q(t), \dot{q}(t), t)$\footnote{Be careful with notation here. Dots like $\dot{q}$ denote time derivatives, while primes like $q'$ denote a transformed quantity. You should pause and convince yourself that the notation $\hat{q}'$ is unambiguous, \ti{i.e.}, that the transformed value of the time derivative of the transformed coordinate.}. We saw in lecture that this is enough to ensure that $q_\alpha \to q_\alpha'$ takes extrema of the action to extrema of the action, and thus takes physical solutions to physical solutions.\\
\\
\
...
\\
\
Since we have parametrized our transformation so that $\lambda = 0$ corresponds not changing anything we know that $G_\alpha(q,0) = g_\alpha$. So setting $\lambda = \epsilon$ and working out to leading order gives an expansion of the form \begin{align*}
G_\alpha(q, \epsilon) & \approx G_\alpha(q,0) + \epsilon \left. \frac{d G_\alpha(q,\lambda)}{\lambda} \right|_{\lambda = 0} \\
...
\ & = \LP m \dot{q}_1 \RP \LP -q_2 \RP + \LP m \dot{q}_2 \RP \LP q_1 \RP + \LP m \dot{q}_1 \RP \LP 0 \RP \\
\ & = m (x \dot{y} - y \dot{x}) \\
\ & = x p_y - y p_x \\
\ & = - \LP \vec{x} \times \vec{p} \RP_z \\
\ & = - L_z.
\end{align*}
Since $Q$ being conserved is exactly the same statement as $-Q$ being conserved, we can be a bit glib and say that the minus sign here isn't important. The moral is that invariance under rotation about the $z$ axis implies conservation of the $z$ component of angular momentum! This suggests several immediate generalizations, which are good to do by hand but should at least by meditated on. \\
\\
\
...
\\
\
To fix this problem, we sometimes say that we \ti{promote} time from an operator to an observable. Geometrically, this means that instead of describing a trajectory with a vector-valued function $\vec{x}(t)$ moving through space \begin{equation*}
